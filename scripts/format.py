...
def inject_inline_styles(html: str, theme: dict, skip_wrapper: bool = False) -> str:
    ...
    def style_pre(match):
        ...
        # 先剥离 <code class="language-xxx"> 外壳，避免 class 文本漏进正文
        code_match = re.match(r'<code([^>]*)>(.*?)</code>', pre_content, flags=re.DOTALL)
        code_attrs = ""
        code_inner = pre_content
        if code_match:
            code_attrs = code_match.group(1) or ""
            code_inner = code_match.group(2)

        has_language = bool(re.search(r'class="language-', code_attrs))
        ...
        code_inner = protect_spaces(code_inner)
        # 公众号编辑器会吃掉 pre 里的 \n，必须转成 <br> 才能保留换行
        code_inner = code_inner.replace("\n", "<br>")
        # 语法高亮：仅对有语言标记的代码块启用（避免破坏 URL 等纯文本内容）
        if has_language:
            code_inner = _basic_syntax_highlight(code_inner)
        ...
        return (
            ...
            f'{mac_header}'
            f'<pre style="{pre_style}"><code style="{pre_code_style}">{code_inner}</code></pre>'
            f'</section>'
        )
...
def generate_gallery(rendered_map: dict, theme_map: dict,
    ...
    # 写入选中主题到临时文件（默认第一个）
    selected_path = selection_file(WORKSPACE_ROOT)
    selected_path.parent.mkdir(parents=True, exist_ok=True)
    selected_path.write_text(default_theme, encoding="utf-8")
...
def format_for_output(content: str, input_path: Path, theme: dict,
    ...
    content = process_img_markers(content, input_path.parent, output_dir)
...
def main():
    ...
    if args.gallery:
        ...
        if AUTO_OPEN and not args.no_open and sys.stdout.isatty():
            webbrowser.open(f"file://{gallery_path}")
            print("已在浏览器中打开画廊")

        print(f"\n完成! 选中主题后点「用这个风格排版」即可复制到剪贴板。")
        print(f"选中的主题 ID 会写入 {selection_file(WORKSPACE_ROOT)}")
        return
