...
- **写任何正文前必读 `prompts/quality-and-risk.md`**（信息增量硬标准 + 去标题党 + 风险软规避 + 去同质化 + 去AI味检查）。
...
- 早/晚定时任务提示词以 `cron-prompts/` 的 v3.2 compact 版本为维护镜像；实际运行配置如存在 `~/.hermes/cron/jobs.json`，需用 cronjob 管理工具或脚本同步，避免 live prompt 停留在旧版。
...
- **去同质化**：重点检查同账号近期内容。发布前比对 publish-history，同一账号近 7 天内主题、标题句式、正文结构、封面构图、正文图类型高度相似则换题、换结构或重做视觉；双账号同日选题也必须在事件/主题层面不同。
...
- **不要固定正文骨架**：信息组件必须完整（事实、多源、分析、读者价值、风险边界），但段落顺序必须按题材重组。禁止每天套"事实底座 → 背景解释 → 原创分析 → 普通人影响 → 中国变量 → 结论"。
- **结构原型轮换**：写作前给本篇选择一个结构原型，并避开同账号近 7 天已用原型。可选：事件拆解型、案例切入型、对比型、问答型、时间线型、清单型、反常识型、现场观察型。
- **小标题内容化**：禁止用"事实底座 / 背景解释 / 原创分析 / 普通人影响 / 中国变量 / 克制结论 / 这件事意味着什么"等通用模板标题。小标题必须写成内容专属判断或问题。
- 双账号默认差异：
  - `xiaocong` / 熵增时刻：科技、产业、AI、公司变化、行业变化。信息密度高、克制、有产业链视角、有数据和对比、少情绪多判断，像一个懂产业的朋友在解释复杂变化。常用但不固定的组织方式：产业链拆解、技术路线对比、数据冲击、时间线复盘、反常识解释、公司动作背后的系统变化。
  - `yeluzi` / 思想的野路子丶：民生、消费、职场、平台规则、普通人生活。更生活化、更有场景感，但不煽情、不站队、不制造对立，要有普通人的切身感。常用但不固定的组织方式：具体场景切入、规则问答、平台/用户对比、普通人行动清单、账单/订单/小店案例、变化前后对照。
...
封面不能长期“同模板换字”。Cron 自动任务默认要先选封面原型并避开同账号近 7 天重复；Swiss Minimal 只作为手动快速封面或封面服务失败时的备用：
...
封面原型建议轮换：纯标题排版型（备用，不连续用）、数据冲击型、结构地图型、场景切入型、对比型、物件隐喻型、时间线型、抽象角色型。封面保持账号品牌气质，但构图、标题排布、视觉主体和主色比例不能短期重复。

需要完整商业编辑风封面时，用 `render_editorial_cover.py`。

**视觉相似性检查不识图**：不要要求 agent 看图判断相似。生成封面/正文图前，必须在 job 目录写 `visual-meta.json`，用生图提示词元数据记录视觉意图，供 `publish_pipe.py` 做同账号近期相似性 warning：
...
- **低创作度相似性扫描**：`--input` 模式下会生成结构/标题/封面签名，并与同账号近期 `publish-history.jsonl` 比对；默认 warning 级不阻断，cron 自动任务必须加 `--fail-on-low-quality-warning`，命中时直接停下，重写结构、标题句式或重做封面/正文图后再推草稿箱。
...
- **低创作度去重字段**：每次记录必须包含结构原型、标题句式、开头方式、结尾方式、封面原型、封面构图、视觉主体、正文图类型。下一次同账号写作前先读这些字段，避免近 7 天重复。
